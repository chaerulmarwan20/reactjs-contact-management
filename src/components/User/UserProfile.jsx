...
export default function UserProfile() {
  ...
  async function fetchUserDetail() {
    ...
    console.log("fetchUserDetail response", response);
    console.log("fetchUserDetail responseBody", responseBody);

    if (response.status === 200) {
      setName(responseBody?.data?.name || "");
    } else {
      await alertError(responseBody?.errors || "Failed to fetch user detail");
    }
  }
  ...
  async function handleSubmitProfile(e) {
    ...
    console.log("handleSubmitProfile response", response);
    console.log("handleSubmitProfile responseBody", responseBody);

    if (response.status === 200) {
      await alertSuccess("Profile updated successfully");
    } else {
      await alertError(responseBody?.errors || "Failed to update user profile");
    }
  }
  ...
  async function handleSubmitPassword(e) {
    ...
    console.log("handleSubmitPassword response", response);
    console.log("handleSubmitPassword responseBody", responseBody);
    ...
    if (response.status === 200) {
      ...
      await alertError(
        responseBody?.errors || "Failed to update user password",
      );
    }
  }

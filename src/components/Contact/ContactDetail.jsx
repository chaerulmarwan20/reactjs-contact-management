...
export default function ContactDetail() {
  ...
  async function fetchContact() {
    ...
    console.log("fetchContact responseBody", responseBody);

    if (response.status === 200) {
      const contact = responseBody?.data;
      setContact(contact);
    } else {
      await alertError(responseBody?.errors || "Failed to fetch contact");
    }
  }
  ...
  async function fetchAddresses() {
    ...
    console.log("fetchAddresses responseBody", responseBody);

    if (response.status === 200) {
      const addresses = responseBody?.data;
      setAddresses(addresses);
    } else {
      await alertError(responseBody?.errors || "Failed to fetch addresses");
    }
  }
  ...
  async function handleDeleteAddress(addressId) {
    ...
    console.log("handleDeleteAddress responseBody", responseBody);

    if (response.status === 200) {
      await alertSuccess("Address deleted successfully");
      fetchAddresses();
    } else {
      await alertError(responseBody?.errors || "Failed to delete address");
    }
  }

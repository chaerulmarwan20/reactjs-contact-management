...
export default function ContactList() {
  ...
  async function handlePageChange(_page) {
    setPage(_page);
    setReload(!reload);
  }
  ...
  async function fetchContacts() {
    ...
    console.log("fetchContacts responseBody", responseBody);

    if (response.status === 200) {
      setContacts(responseBody?.data || []);
      setTotalPage(responseBody?.paging?.total_page || 0);
    } else {
  ...
  async function handleContactDelete(id) {
    ...
    console.log("handleContactDelete response", response);
    console.log("handleContactDelete responseBody", responseBody);

    if (response.status === 200) {
      await alertSuccess("Contact deleted successfully");
      setReload(!reload);
    } else {
      await alertError(responseBody?.errors || "Failed to delete contact");
    }
  }
